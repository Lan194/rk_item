...
static void send_ptu_ack(uint8_t func,uint8_t result)
{
    ...
    uint8_t crc = crc8_calc(ack,7);
    ack[7]=crc;
    uart_send_data(ack,8);
}

/*从环形buf取一字节，成功返回0，空返回-1*/
...
/*环形缓冲区读指针前进 n 字节（用于跳过残留帧头）*/
static void ring_advance(uint16_t ring_size, uint16_t *p_rd, uint16_t n)
{
    *p_rd = (uint16_t)((*p_rd + n) % ring_size);
}
...
/*连续半包等待计数——防止活锁：如果一个半包等了太多次迭代还没来齐，
  就跳过那个假帧头重新同步*/
static int partial_stall = 0;
#define PARTIAL_STALL_LIMIT 5  /*5次=约500ms，超过就放弃这个半包*/
...
void protocol_parse_ptu_cmd(uint8_t *ring, uint16_t ring_size, uint16_t *p_rd, uint16_t *p_wr)
{
    ...
    while(0 == ring_get_byte(ring, ring_size, p_rd, p_wr, &b))
    {
        ...
        if(0 != ring_get_byte(ring, ring_size, p_rd, p_wr, &b))
        {
            ...
            {
                /*放弃这个假帧头，rd 前进 1 字节继续扫描*/
                ring_advance(ring_size, p_rd, 1);
                partial_stall = 0;
                LOG_I("partial frame stall, skip stale 0x55\n");
                continue;
            }
            *p_rd = frame_start;
            return;
        }
        partial_stall = 0; /*拿到完整帧头，重置 stall 计数*/
        ...
        uint8_t crc_calc = crc8_calc(frame_buf, frame_len - 1);
